...
export const sendVerificationCodeEmail = async (email: string, code: string): Promise<boolean> => {
  try {
    ...
    // First try SMTP (more reliable with your current config)
    ...
    // Try Resend as fallback only if SMTP failed
    ...
    console.warn('No email service configured properly. SMTP credentials missing and Resend API key unavailable.')
    console.log('For testing purposes, verification code for', email, ':', code)
    return false
